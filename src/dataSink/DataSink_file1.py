...
class FileOutput:
    ...
    def __init__(self, config: SnapVariables, centre_frequency_hz: float, sample_rate_sps: float):
        ...
        # check we don't go over the max file size we are allowing (100MBytes), 4bytes per IQ sample
        max_file_size = 100e6
        if (self._max_total_samples * 4) > max_file_size:
            self._max_total_samples = max_file_size / 4
    ...
    def get_pre_trigger_milliSeconds(self) -> float:
        return self._pre_milliseconds

    def get_post_trigger_milliSeconds(self) -> float:
    ...
    def _start(self) -> None:
        ...
        try:
            ...
            # TODO: number of samples should be in fft size so just add in 32768 for now
            size = int(self._max_total_samples + 32768)
            self._complex_post_data = np.empty(shape=(size,), dtype=np.complex64)
            self._post_index = 0
    ...
    def _end(self) -> None:
        ...
        """
        if self._file:
            self._file.close()  # would of normally deleted the temporary file, but we added delete=False
    ...
    def write(self, trigger: bool, data: np.array) -> bool:
        ...
        if self._file:
            if (self._max_total_samples - self._number_samples_written) > 0:
                ...
                # TODO: fix TypeError: copyto() argument 1 must be numpy.ndarray, not numpy.complex64
                #np.copyto(self._complex_post_data[self._post_index], data)
                #self._post_index += data.size
