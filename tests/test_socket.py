...
def test_socket_connection_fails():
    sock = DataSource_socket.Input(parameters="test_missing_lookup:1",
                                   data_type="16tbe",
...
def test_socket_connection_no_server():
    sock = DataSource_socket.Input(parameters="127.0.0.1:1",
                                   data_type="16tle",
                                   sample_rate=1.0, centre_frequency=1.0, input_bw=1.0)
...
def test_socket_too_few_parts():
    sock = DataSource_socket.Input(parameters="127.0.0.1",
                                   data_type="16tle",
                                   sample_rate=1.0, centre_frequency=1.0, input_bw=1.0)
...
def test_socket_bad_port_number():
    sock = DataSource_socket.Input(parameters="127.0.0.1:abc",
                                   data_type="16tle",
                                   sample_rate=1.0, centre_frequency=1.0, input_bw=1.0)
    with pytest.raises(Exception):
        sock.open()
...
def test_socket_is_a_server():
    server = DataSource_socket.Input(parameters=":123",
                                     data_type="16tle",
                                     sample_rate=1.0, centre_frequency=1.0, input_bw=1.0)
    server.open()
...
def test_read_cplx_samples():
    ...
    sock = DataSource_socket.Input(parameters="192.168.0.12:5555",
                                   data_type="16tr",
                                   sample_rate=1.0, centre_frequency=1.0, input_bw=1.0)
    sock.open()
