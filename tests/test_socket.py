...
def test_socket_too_few_parts():
    sock = DataSource_socket.Input(ip_address_port="127.0.0.1", number_complex_samples=100,
                                   data_type="16tle",
                                   sample_rate=1.0, centre_frequency=1.0, input_bw=1.0)
...
def test_socket_bad_port_number():
    sock = DataSource_socket.Input(ip_address_port="127.0.0.1:abc", number_complex_samples=100,
                                   data_type="16tle",
                                   sample_rate=1.0, centre_frequency=1.0, input_bw=1.0)
    with pytest.raises(Exception):
        sock.open()
